...
module internal ReactComponentHelpers =
    ...
    let private injectReactNoMemo noMemo body =
        let noMemo = defaultArg noMemo false
    ...
    let applyImportOrMemo import from memo noMemo (decl: MemberDecl) =
        ...
        | _, _, Some true ->
            ...
            let body =
                ...
                |> injectReactImport
                |> fun body -> [
                    Delegate(decl.Args, body |> injectReactNoMemo noMemo, None, Tags.empty)
                ]
        ...
        | _ -> {
            decl with
                Body = decl.Body |> injectReactImport |> injectReactNoMemo noMemo
          }
...
type ReactComponentAttribute(?exportDefault: bool, ?import: string, ?from: string, ?memo: bool, ?noMemo: bool) =
    ...
    override this.Transform(compiler, file, decl) =
        ...
        if info.IsValue || info.IsGetter || info.IsSetter then
            ...
            let errorMessage =
                sprintf "Expected function %s to return a ReactElement when using [<ReactComponent>]. Instead it returns %A" decl.Name decl.Body.Type
            ...
            if decl.Args.Length = 1 && AstUtils.isRecord compiler decl.Args[0].Type then
                ...
                |> applyImportOrMemo import from memo noMemo
...
type ReactMemoNoMemoComponentAttribute(?exportDefault: bool) =
    inherit ReactComponentAttribute(?exportDefault = exportDefault, ?import = None, ?from = None, memo = true, noMemo = true)
    new() = ReactMemoNoMemoComponentAttribute(exportDefault = false)

type ReactNoMemoComponentAttribute(?exportDefault: bool, ?import: string, ?from: string) =
    inherit ReactComponentAttribute(?exportDefault = exportDefault, ?import = import, ?from = from, memo = false, noMemo = true)
    new(exportDefault: bool) = ReactNoMemoComponentAttribute(exportDefault = exportDefault, ?import = None, ?from = None)
    new(import: string, from: string) = ReactNoMemoComponentAttribute(exportDefault = false, import = import, from = from)
